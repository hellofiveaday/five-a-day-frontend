...
export default function WhatsAppPopup() {
  ...
  useEffect(() => {
    ...
    if (location.pathname === "/") {
      // Home: show 3 s after scrolling past the yellow banner
      let handled = false;
      const onScroll = () => {
        if (handled) return;
      ...
      // Other pages: show after 10 s from arrival
  ...
  return (
    <div
      className="fixed bottom-6 right-6 z-50 bg-white rounded-3xl shadow-2xl p-5 flex gap-4 items-start"
      style={{
    ...
    >
      ...
      <a href={siteConfig.whatsapp} target="_blank" rel="noopener noreferrer" className="shrink-0">
        <div className="w-11 h-11 bg-[#25D366] rounded-full flex items-center justify-center shadow-md">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-gray-800 text-sm mb-1.5">¿Tienes alguna pregunta?</p>
        ...
        <div className="flex justify-end">
          <a
            ...
            className="inline-block bg-[#25D366] hover:bg-[#1ebe5d] text-white text-xs font-semibold py-2 px-5 rounded-full transition-colors"
          >
            Enviar mensaje
          </a>
        </div>
